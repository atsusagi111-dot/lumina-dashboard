...
-- AI 分析レポート：1 人の 1 か月につき 1 件（unique で重複を防ぐ）。
-- 取り込みではなく人と月に紐づける理由：取り込みは毎月やり直すため、
-- 取り込みに紐づけると前月までの分析が見えなくなるため。
create table if not exists public.reports (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null default auth.uid() references auth.users (id) on delete cascade,
  -- どの取り込みのデータで作ったかの記録。取り込みが消えても分析は残す
...
-- reports の (user_id, target_month) は unique 制約が自動で索引を作るため、別途の索引は不要
