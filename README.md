...
「各自のアカウントでログインしてチーム全員が同じデータを見る」形と、**PDF でのレポート出力**は
[§11 Phase 2 ロードマップ](#11-phase-2-ロードマップ) に入れています。
...
| --- | --- | --- | --- |
| 高 | **PDF レポート出力**（画面の「PDF で保存」ボタン） | 閲覧者への配布はスクリーンショットで当面まかなえるため。実装は印刷用 CSS + `window.print()` で、追加ライブラリ・追加費用なし | 印刷用のレイアウト調整（半日程度） |
| 高 | **チーム共有**（各自のアカウントで同じデータを見る） | MVP は「取り込んだ本人だけが見える」RLS で十分と判断したため（[§5-5](#5-5-ログインとデータの守り方)） | 組織単位の RLS 設計、招待の仕組み |
...
| 中 | 異常検知アラート（前月比 ±20% で Slack 通知） | 「見に行けば分かる」段階のため後回し。定期実行の仕組みが別途要る | Slack の Webhook、Vercel Cron |
| 中 | PDF レポート自動生成 + メール配信 | 画面のスクリーンショットで当面まかなえるため | PDF 生成、メール配信サービス |
